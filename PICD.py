...
def save_community_partition(dataset_name, output_folder, k, readPath):
    ...
    para = [ecd.centers, ecd.current_radii, ecd.objective, ecd.overlap_ratio, ecd.uncovered_ratio, ecd.num_overlap_nodes, ecd.num_uncovered_nodes, ecd.overlap_nodes, ecd.uncovered_nodes]
    return (dataset_name, duration_ms, para)
...
if __name__ == '__main__':
    ...
    # 输出文件夹
    output_folder = './ours-result/real'
    ...
    for name in names:
        ...
        current_name, time_ms, para = save_community_partition(name, output_folder, k, readPath)
        processing_times.append((current_name, time_ms, para))

    # 将时间和指标记录到 CSV 文件（保存到 'real' 子目录）
    csv_filename = f'{output_folder}/processing_times_and_metrics.csv'
    fieldnames = ['Dataset Name', 'Time (ms)', 'Centers', 'Radii', 'Objective', 'Overlap Ratio', 'Uncovered Ratio', 'Num Overlap Nodes',
                  'Num Uncovered Nodes', 'Overlap Nodes', 'Uncovered Nodes']
    ...
    with open(csv_filename, 'w', newline='') as f:
        ...
        # 写入数据
        for name, time_ms, para in processing_times:
            row = [name, time_ms] + para
            writer.writerow(row)

    print(f"\n所有社区检测完成！时间和指标记录已保存至 '{csv_filename}' 文件中。")
